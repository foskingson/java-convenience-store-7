...
- [x] 입력
    - [x] 상품 목록과 행사 목록을 파일 입출력을 통해 가져온다.
    - [x] 구매할 상품과 수량을 입력받는다.
...
- [x] 출력
    ...
    - [x] 영수증을 출력한다. 구매 상품 내역, 증정 상품 내역, 금액 정보를 담고 있다. 
...
1. 함수의 길이를 기준으로 너무 길어지지 않게 역할을 명확하게 분리한다.

2. 다양한 예외 상황에 대해 고민하는 습관을 들인다.

3. 비즈니스 로직과 UI를 분리한다. 비즈니스 로직과 UI 로직을 한 클래스에서 처리하는 것은 단일 책임 원칙(SRP)에 위배된다. 

4. 연관성이 있는 상수는 static final 대신 enum을 활용한다.

5. final 키워드를 사용해 값의 변경을 막는다.

6. 객체의 상태 접근을 제한한다. 접근제어자를 private으로 설정하여 캡슐화의 원칙을 지킨다.

7. 객체에서 데이터를 getter해서 처리하지 말고 객체 내에서 스스로 처리할 수 있도록 한다.

8. 필드(인스턴스 변수)의 수를 줄이기 위해 노력한다.

9. 성공하는 케이스 뿐만 아니라 예외 케이스도 테스트한다.

10. 테스트 코드도 코드다. 지속적인 리팩터링을 통해 개선해나가는 것이 중요하다.

11. 테스트를 위한 코드는 구현 코드에서 분리되어야 한다. 테스트를 쉽게 하기 위해 접근제어자를 변경해버리면 구현 코드가 테스트에 종속된다.

12. 단위 테스트하기 어려운 코드를 단위 테스트하기 쉽게 만드는 방법은 테스트하기 어려운 의존성을 외부에서 주입하거나 분리하여 테스트 가능한 상태로 만드는 것이다.
``` java
public class Calculator {
    ...
    private int add(int a, int b) { // private 메서드라서 테스트가 어려움
        return a + b;
    }
}

// ==> 테스트하기 어려운 의존성을 외부에서 주입하여 사용
